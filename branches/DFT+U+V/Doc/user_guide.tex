...
Plug-ins \texttt{YAMBO}, \texttt{PLUMED}, \texttt{W90}, \texttt{WanT} should be downloaded into
...
\item \texttt{make ph}  compiles the \PHonon\
package
...
\item \texttt{make ld1} compiles the pseudopotential package \texttt{atomic} s
...
\item \texttt{make plumed} unpacks \texttt{PLUMED}, patches several routines
                           ...
                           recompiles \PWscf\ and \CP\ with \texttt{PLUMED} support.
...
To run the examples, you should edit file
\texttt{examples/environment\_variables}, 
...
For execution using OpenMP on N threads, 
set  PARA\_PREFIX to \texttt{env OMP\_NUM\_THREADS=N}.

begin************************************************************************

To run a single example, go to the corresponding
directory (e.g.
 \texttt{examples/example01}) and execute: 
\begin{verbatim}
      ./run_example
\end{verbatim}
...
large multicore parallel machines. Explicit OpenMP can run together with MPI and also together 
with multithreaded libraries. Beware conflicts between the various kinds of
...
In parallel execution, each processor has its own slice of wavefunctions, that have to be written
...
\item ``collected'': wavefunctions are
...
\end{itemize}
\CP\ always uses the latter approach;
\PWscf\ (and all other packages) the former, 
but the final data can be written in ``collected''
format if the input variable \texttt{wf\_collect}
...
processors (i.e., you cannot use local scratch directories: there is presently 
no way to ensure that the distribution of processes on processors will follow the same pattern 
for different jobs). 

Files written by \PWscf\ can be read by \CP\ only if \texttt{wf\_collect=.true.} (and if
...
scf calculation with \pwx, for Norm-Conserving pseudopotentials. Ultrasoft pseudopotentials
nd PAW 
...
\texttt{ngm}), and 
$T_{us}$ = time required by PAW/Ultrasoft contribution (if any).
Note that for Norm-Conserving pseudopotentials
the FFT grids for charge and
...
of the FFT grids, \texttt{nr3} and \texttt{nr3s}, which coincide for Norm-Conserving pseudopotentials);
...
\begin{verbatim}
   mpiexec -n 16 pw.x -npool 8 -in input
\end{verbatim}
SGI Altix and PC clusters using \texttt{mpirun}:
\begin{verbatim}   mpirun -np 16 pw.x -npool 8 -in input
\end{verbatim}
