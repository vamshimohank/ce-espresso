...
MODULE xml_io_base
  ...
  CONTAINS
    ...
    SUBROUTINE write_xc( dft, nsp, lda_plus_u, &
                         Hubbard_U, Hubbard_alpha, Hubbard_beta, &
      ...
      REAL(DP), OPTIONAL, INTENT(IN) :: Hubbard_U(:,:,:), Hubbard_alpha(:,:), Hubbard_beta(:,:)
      ...
      IF ( lda_plus_u ) THEN
         !
         IF ( .NOT. PRESENT( Hubbard_U )    .OR. &
              .NOT. PRESENT( nsp )          .OR. &
              .NOT. PRESENT( Hubbard_beta ) .OR. &
              .NOT. PRESENT( Hubbard_alpha ) ) &
            CALL errore( 'write_exchange_correlation', &
                         ' variables for LDA+U not present', 1 )
         !
         CALL iotk_write_dat( iunpun, "NUMBER_OF_SPECIES", nsp )
         CALL iotk_write_dat( iunpun, "HUBBARD_U", Hubbard_U(1:nsp,0:lmaxx,0:lmaxx) )
         CALL iotk_write_dat( iunpun, "HUBBARD_ALPHA", Hubbard_alpha(1:nsp,0:lmaxx) )
         CALL iotk_write_dat( iunpun, "HUBBARD_BETA", Hubbard_beta(1:nsp,0:lmaxx) )
         !
      END IF
