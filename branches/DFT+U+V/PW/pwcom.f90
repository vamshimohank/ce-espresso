...
MODULE ldaU
  ...
  !
  USE kinds,      ONLY : DP
  USE parameters, ONLY : lqmax, ntypx, lmaxx
  !
  SAVE
  !
  INTEGER, PARAMETER :: nspinx=2
  COMPLEX(DP), ALLOCATABLE :: swfcatom(:,:)    ! orthogonalized atomic wfcs
  REAL(DP) :: eth                              ! the Hubbard contribution to the energy
  REAL(DP) :: Hubbard_U(ntypx,0:lmaxx,0:lmaxx) ! the on-site, multiorbital Hubbard U
  REAL(DP) :: Hubbard_alpha(ntypx,0:lmaxx)     ! the Hubbard alpha (used to calculate U)
  REAL(DP) :: Hubbard_beta(ntypx,0:lmaxx)      ! the Hubbard beta (alpha + spin*beta, spin = -1,+1)
  REAL(DP) :: starting_ns(lqmax,nspinx,ntypx)  ! TODO: modify this
  INTEGER ::  niter_with_fixed_ns              ! no. of iterations with fixed ns
  ! not needed anymore
  !!INTEGER ::  Hubbard_l(ntypx)                 ! the angular momentum of Hubbard states
  !!INTEGER ::  Hubbard_lmax = 0                 ! maximum angular momentum of Hubbard states
  LOGICAL :: lda_plus_u                        ! .TRUE. if lda+u calculation is performed
  LOGICAL :: conv_ns                           ! .TRUE. if ns are converged
  CHARACTER(len=30) :: U_projection            ! 'atomic', 'ortho-atomic', 'file'
  INTEGER, ALLOCATABLE :: oatwfc(:,:)          ! offset of atomic wfcs used for projections (natoms,0:lmaxx)
  CONTAINS
  integer function ilm(l,m)                    ! l = 0..lmax, m = 1..2*l+1
