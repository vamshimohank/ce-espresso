...
CONTAINS
 ...
 nullify (rho%kin_r, rho%kin_g, rho%ns, rho%bec)
   ...
   if (lda_plus_u) allocate (rho%ns((lmaxx+1)**2,(lmaxx+1)**2,nspin,nat))
 ...
 subroutine diropn_mix_file( iunit, extension, exst )
   ...
   if (lda_plus_u)     rlen_ldaU = (lmaxx+1)**4 *nspin*nat
...
FUNCTION ns_ddot( rho1, rho2 )
  ...
  USE kinds,     ONLY : DP
  USE parameters,ONLY : lmaxx
  USE ldaU,      ONLY : Hubbard_U, oatwfc
  ...
  do na = 1, nat
    ...
    do il = 0, lmaxx
      ...
      i0 = il*il + 1
      i1 = i0 + 2*il
      ns_ddot = ns_ddot + 0.5d0 * hubbard_U(nt,il,il) * &
                sum( rho1%ns(i0:i1,i0:i1,1:nspin,na) * rho2%ns(i0:i1,i0:i1,1:nspin,na) )
    enddo
  enddo
