...
!  IF ( do_comp )  CALL write_ee_summary()
  ...
  !   DFT+U+V
  !
  if (lda_plus_U) call Hubbard_summary
  !
...
!======================================================================
! Print a summary of the DFT+U_V input parameters
!======================================================================
SUBROUTINE Hubbard_summary
  use io_global, only : stdout
  use ions_base, only : ntyp => nsp, atm
  use constants, only : rytoev
  use ldaU
  implicit none
  character, dimension(4), parameter :: spdf = (/'s', 'p', 'd', 'f'/)
  integer :: nt, nt1, nt2, il, il1, il2

  write(stdout,*)
  write(stdout,'(5X,''DFT+U+V calculation: --------------------------------------------'')')

  write(stdout,'(5X,''On-site interactions:'')')
  do nt = 1, ntyp
    do il1 = 0, lmaxx
      do il2 = 0, lmaxx
        if (Hubbard_U(nt,il1,il2) < 1d-6) cycle
        write(stdout,'(5X,A3,1X,A1,4X,A3,1X,A1,6X,''U ='',F12.6)') &
              atm(nt), spdf(il1+1), atm(nt), spdf(il2+1), Hubbard_U(nt,il1,il2)*rytoev
      enddo
    enddo
  enddo

#if 0
  write(stdout,'(5X,''Pair interactions:'')')
  do nt1 = 1, ntyp
    do nt2 = 1, ntyp
      do il1 = 0, lmaxx
        do il2 = 0, lmaxx
          if (Hubbard_V(nt1,il1,nt2,il2) < 1d-6) cycle
          write(stdout,'(5X,A3,1X,A1,4X,A3,1X,A1,6X,''V ='',F12.6,4X,''rc='',F10.4)') &
                atm(nt1), spdf(il1+1), atm(nt2), spdf(il2+1), &
                Hubbard_V(nt1,il1,nt2,il2)*rytoev, Hubbard_V_cutoff(nt1,il1,nt2,il2)
         enddo
       enddo
     enddo
   enddo
  ...
  write(stdout,'(5X,''Individual orbital shifts:'')')
  do nt = 1, ntyp
    do il = 0, lmaxx
      if (abs(Hubbard_alpha(nt,il)) < 1d-6 .and. abs(Hubbard_beta(nt,il)) < 1d-6) cycle
      write(stdout,'(5X,A3,1X,A1,4X,''alpha ='',F12.6,4X,''beta ='',F12.6)') &
            atm(nt), spdf(il+1), Hubbard_alpha(nt,il)*rytoev, Hubbard_beta(nt,il)*rytoev
    enddo
  enddo
  write(stdout,'(5X,''-----------------------------------------------------------------'')')
  write(stdout,*)

END SUBROUTINE Hubbard_summary
