...
SUBROUTINE v_hubbard(ns, v_hub, eth)
  ...
  USE ldaU,                 ONLY : Hubbard_U, Hubbard_alpha, Hubbard_beta, ilm
  USE lsda_mod,             ONLY : nspin

  IMPLICIT NONE
  !
  REAL(DP), INTENT(IN)  :: ns((lmaxx+1)**2,(lmaxx+1)**2,nspin,nat) 
  REAL(DP), INTENT(OUT) :: v_hub((lmaxx+1)**2,(lmaxx+1)**2,nspin,nat) 
  REAL(DP), INTENT(OUT) :: eth
  REAL(DP) :: spin
  INTEGER :: is, na, nt, m1, m2, il
  !
  ...
  DO na = 1, nat  
    ...
    DO il = 0, lmaxx
      ...
      DO is = 1, nspin
        ...
        DO m1 = 1, 2*il+1
          eth = eth + ( Hubbard_alpha(nt,il) + spin*Hubbard_beta(nt,il) + 0.5D0 * Hubbard_U(nt,il,il) ) * &
                ns(ilm(il,m1),ilm(il,m1),is,na) 
          v_hub(ilm(il,m1),ilm(il,m1),is,na) = v_hub(ilm(il,m1),ilm(il,m1),is,na) + &
                ( Hubbard_alpha(nt,il) + spin*Hubbard_beta(nt,il) + 0.5D0 * Hubbard_U(nt,il,il) ) 
          DO m2 = 1, 2*il+1
            eth = eth - 0.5D0 * Hubbard_U(nt,il,il) * &
              ns(ilm(il,m2),ilm(il,m1),is,na) * ns(ilm(il,m1),ilm(il,m2),is,na) 
            v_hub(ilm(il,m1),ilm(il,m2),is,na) = v_hub(ilm(il,m1),ilm(il,m2),is,na) - &
              Hubbard_U(nt,il,il) * ns(ilm(il,m2),ilm(il,m1),is,na) 
          ENDDO
        ENDDO
      ENDDO
    ENDDO
